...
class QueryResolverImpl2 extends QueryResolver {
  ...
  def hero(episode: EpisodeDO): CharacterDO = {
    ...
    val characterResponseProjection = new CharacterResponseProjection().selectAll$(1)
  ...
  def human(id: String): HumanDO = {
    ...
    //must use typename, and add jackson annotation to support, since v2.4
    val humanResponseProjection = new HumanResponseProjection().selectAll$(1)
  ...
  def droid(id: String): DroidDO = {
    ...
    //must use typename, and add jackson annotation to support, since v2.4
    val droidResponseProjection = new DroidResponseProjection().selectAll$(1)
