...
class GraphQLCodegenRestrictedWordsTest {
    ...
    void generate() throws Exception {
        ...
        List<String> waitAsserts = Arrays.asList("Char.kt", "Super.kt","CharResponseProjection.kt", "FunQueryRequest.kt","" +
                "FunQueryResponse.kt", "QueryFunParametrizedInput.kt", "TestEnum.kt", "WhenQueryAPI.kt");
        List<String> generatedFileNames = Arrays.stream(files).map(File::getName).filter(waitAsserts::contains).sorted().collect(toList());
        assertEquals(waitAsserts.stream().sorted().collect(toList()), generatedFileNames);

        for (File file : files) {
            if (waitAsserts.contains(file.getName())) {
